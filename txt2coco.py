...
coco_format_save_path='F:\\txt2json\\json\\'   #要生成的标准coco格式标签所在文件夹
yolo_format_classes_path='F:\\txt2json\\names.txt'     #类别文件，一行一个类
yolo_format_annotation_path='F:\\txt2json\\labels\\'  #yolo格式标签所在文件夹
img_pathDir='F:\\txt2json\\images\\'    #图片所在文件夹
...
for i,imageFile in enumerate(imageFileList):
    ...
    txtFile=imageFile[0:len(imageFile)-4]+'.txt'
    #txtFile = imageFile[]                                               #获取该图片获取的txt文件
...
name = os.path.join(coco_format_save_path + '.json')
with open(name,'w') as fw:     
    json.dump(write_json_context,fw,indent=2)                                            #将字典信息写入.json文件中